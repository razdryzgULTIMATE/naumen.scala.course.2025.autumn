...
object Exercises extends App {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom to iTo).foldRight(0)((x, sum) => if (x % 3 == 0 || x % 5 == 0) sum + x else sum)
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        @tailrec
        def loop(currentNum: Int, prob: Int, acc: Set[Int]): Set[Int] = {
            currentNum match {
                case 1 => acc
                case x => {
                    if (currentNum % prob == 0) {
                        loop(x / prob, prob, acc + prob)
                    }
                    else {
                        loop(x, prob + 1, acc)
                    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        val m = (r: Int, d: Double) => (4.0/3.0) * pi * r * r * r * d
        ballsArray
            .toSeq
            .map{ case (name, (r,d)) => name -> m(r,d)}
            .sortBy(_._2) // сортируем по массе
            .map(_._1)
    }
